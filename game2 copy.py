...
def safe_tan_half(α):
        ε = 0.05
        return np.tan(α / 2) if abs(α) > ε else (α / 2) * (1 + (α**2 / 12))
...
def compute_next_state(state: State, dα, time_val):
    ...
    sol = solve_ivp(ODEeq, [t1, t2], state_list, args=(dα,), method='LSODA')
...
class SnakeRobotGame:
    def __init__(self):
        ...
        self.time_span = np.arange(0,500,0.1)
        self.time_val = [0]
        self.counter = 0
    ...
    def play_step(self):
        ...
        self.dα =  -1.3* np.cos(self.time_val[-1])
        ...
        test = self.sim.history["p"]
        print(f"Time = {self.time_val[-1]}, p: {test[-1]}")
    ...
    def _update_ui(self, action_idx):
        ...
        #print(f"Time = {self.time_val[-1]}, x: {state.x}, y: {state.y}")
        x0 = state.x - λ/2 * np.cos(state.θ)
        y0 = state.y - λ/2 * np.sin(state.θ)
        x1 = state.x + λ/2 * np.cos(state.θ)
        y1 = state.y + λ/2 * np.sin(state.θ)
        x2 = x1 + λ * np.cos(state.θ + state.α)
        y2 = y1 + λ * np.sin(state.θ + state.α)
...
class SnakeRobotEnv:
    ...
    def _compute_reward(self):
        ...
        if abs(abs(s.α)-np.pi) < 0.2:
            reward -= np.exp(1/abs(abs(s.α)-np.pi))
